...
restoration_db_final <- restoration_db %>%
  ...
  mutate(
    ...
    Restoration_in_agricultural_land = case_when(
      source == "Chaturvedietal_2018" ~ "Y, except grassland, areas under surface and groundwater irrigation, and croplands with more than 40 percent tree canopy cover",
      source == "Bastinetal_2019" ~ "N",
      TRUE ~ "Y"
    ),
    Include_existing_trees = case_when(
      source %in% c("Bastinetal_2019", "ORNL_DAAC", "Chaturvedietal_2018") ~ "Y",
    ...
    Available_map_on_FABLE_Zonal_Statistics_Extractor = case_when(
      source %in% c("Bastinetal_2019", "ORNL_DAAC", "Chaturvedietal_2018",
...
write.xlsx(restoration_db_final,  file = here("data", "output", "restoration_db.xlsx"), row.names = FALSE)
