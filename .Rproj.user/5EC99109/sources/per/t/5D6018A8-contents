...
griscometal_ha <- read_excel(here("data/input/griscometal_ha.xlsx")) %>% 
...
Worldforest_db <- restoration_db %>% 
  ...
  distinct () %>% 
  ungroup() %>% 
  rbind(griscometal_ha)
# %>%
  # mutate(Value = case_when(
  # Variable == "refor_tech_ha_tot" ~
  #     Value - Value[Variable == "refor_feas_ha_tot"],
...
Worldforest_stacked <- Worldforest_db %>% 
  ...
  mutate(
    Group = case_when(
      ...
      Variable %in% c("refor_tech_ha_tot", "refor_feas_ha_tot") ~ "Buschetal",
    ...
    Group_label = case_when(
      ...
      Group == "griscometal" ~ "Griscom et al. (2017)",
      TRUE ~ Group
    ),
    Variable_label = case_when(
      ...
      Variable == "refor_tech_ha_tot" ~ "Technical restoration potential (2020-2050)",
      Variable == "refor_feas_ha_tot" ~ "Cost-effective restoration potential (2020-2050)",
      ...
      Variable ==  "Reforestation" ~ "Tree cover restoration potential"
  ...
  mutate(
    Axis_group = case_when(
      ...
      Group_label == "Busch et al. (2019)" ~ Group_label,  # all Roe together
      TRUE ~ Group_label
    )
  ) %>% 
  mutate(
    Variable_label = factor(
      ...
      levels = c(
        ...
        "Cost-effective restoration potential (2020-2050)",
        "Technical restoration potential (2020-2050)"
      )
    )
  ) %>% 
  mutate(
...
write.xlsx(Worldforest_stacked,  file = here("data", "output", "restoration_world.xlsx"), row.names = FALSE)
...
label_source <- c(
  ...
  "refor_tech_ha_tot" = "Busch et al. (2019)",
  "refor_feas_ha_tot" = "Busch et al. (2019)",
...
bars_data <- Worldforest_stacked %>%
  filter(Variable_label != "Cost-effective restoration potential (2020-2050)")

point_data <- Worldforest_stacked %>%
  filter(Variable_label == "Cost-effective restoration potential (2020-2050)")

World_plot <- ggplot() +
  geom_col(
    data = bars_data,
    aes(x = Axis_group, y = Value, fill = Variable_label),
    position = "stack"
  ) +
  ## cost-effective as point
  geom_point(
    data = point_data,
    aes(x = Axis_group, y = Value, fill = Variable_label),
    shape = 21,
  ...
  scale_fill_manual(
    ...
    values = c(
      ...
      "Technical restoration potential (2020-2050)" = "#1b9e77",
      "Cost-effective restoration potential (2020-2050)" = "#f781bf"
    ),
...
write.xlsx(Indiaforest_stacked,  file = here("data", "output", "restoration_india.xlsx"), row.names = FALSE)


bars_data <- Indiaforest_stacked %>%
  filter(Variable_label != "Cost-effective restoration potential (2020-2050)")

point_data <- Indiaforest_stacked %>%
  filter(Variable_label == "Cost-effective restoration potential (2020-2050)")
...
India_plot <- ggplot() +
  geom_col(
    data = bars_data,
    aes(x = Group_label, y = Value, fill = Variable_label),
    position = "stack"
  ) +
  ## cost-effective as point
  geom_point(
...
bars_data_br <- Brazilforest_stacked %>%
  filter(Variable_label != "Cost-effective restoration potential (2020-2050)")

point_data_br <- Brazilforest_stacked %>%
  filter(Variable_label == "Cost-effective restoration potential (2020-2050)")
...
#     Group = case_when(
#       Variable == "ClimateImpact_treecoverrestoration_total" ~ "Hasleretal_2024",
#       Variable %in% c("ars_feascum", "ars_techcum") ~ "Buschetal",
#       TRUE ~ NA_character_
#     ),
#     Group_label = case_when(
#       Group == "Hasleretal_2024" ~ "Hasler et al (2024)",
#       Group == "Buschetal" ~ "Busch (2019)",
#       TRUE ~ Group
#     )
#   )
# 
...
#   "ars_feascum" = "Busch (2019)",
#   "ars_techcum" = "Busch (2019)"
# )
# 
...
#   filter(Variable %in% c("Restoration_Potential", "Potential_forestregeneration_total", "refor_tech_ha_tot_tot", "refor_feas_ha_tot_tot")) %>%
#   group_by(Variable) %>% 
#   mutate(Value  = sum(Value)) %>% 
#   select(-iso3) %>% 
#   mutate(iso3 = ("WRD")) %>% 
...
#   "refor_tech_ha_tot_tot" = "Roe et  al. (2020)" ,
#   "refor_feas_ha_tot_tot" = "Roe et  al. (2020)"
# )
# 
# variable_labels <- c(
...
#   "refor_tech_ha_tot_tot" = "Total technical reforestation potential (2020-2050)",
#   "refor_feas_ha_tot_tot" = "Total cost-effective reforestation potential (2020-2050)"
# )
# 
...
#       "refor_tech_ha_tot_tot" = "#7570b3",
#       "refor_feas_ha_tot_tot" = "#e7298a"
#     ),
